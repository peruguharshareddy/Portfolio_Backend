...
const SYSTEM_PROMPT = `You are a friendly portfolio assistant for HarshaVardhan. Answer questions about HarshaVardhan's skills, projects, experience, education, certificates, and achievements.
...
async function buildContext() {
  const [skills, projects, experiences, education, certificates, recognitions] =
    await Promise.all([
  ...
  ctx += "## Contact\n";
  ctx +=
    "For direct inquiries, users should use the Contact form on the portfolio.\n";

  return ctx;
}

const fallbackResponses = [
  "Hi! I'm HarshaVardhan's portfolio assistant. Ask me about skills, projects, experience, or anything else about HarshaVardhan!",
  ...
  "I'm here to help you learn about HarshaVardhan's work. Ask me anything about the portfolio!",
];
...
async function generateResponse(message) {
  ...
  if (
    ...
    const greetings = [
      "Hi! I'm HarshaVardhan's portfolio assistant. Ask me about skills, projects, experience, or anything else!",
      "Hello! Welcome to the portfolio. I can help you explore HarshaVardhan's work. What would you like to know?",
    ];
  ...
  try {
    ...
    const formatSkillList = (skills) => {
      ...
      let text = "Here are HarshaVardhan's technical skills:\n\n";
      for (const [cat, items] of Object.entries(grouped)) {
        text += `*${cat}*\n${items.join("\n")}\n\n`;
      }
    ...
    const formatProjectList = (projects) => {
      ...
      let text = `Here are the projects HarshaVardhan has worked on (${projects.length} total):\n\n`;
      ...
      if (projects.length > 5)
        text += `...and ${projects.length - 5} more projects.`;
      return text;
    };
    ...
    const formatExperienceList = (experiences) => {
      ...
      for (const exp of experiences) {
        ...
        if (exp.description)
          text += `   ${exp.description.substring(0, 200)}\n\n`;
      }
      return text;
    };
    ...
    const formatRecognitionList = (recognitions) => {
      ...
      for (const rec of recognitions) {
        text += `*${rec.title}*\n`;
        if (rec.description)
          text += `   ${rec.description.substring(0, 200)}\n\n`;
      }
      return text;
    };
    ...
    if (["contact"].includes(category)) {
      return "You can reach out via the Contact form on this portfolio. HarshaVardhan typically responds within 24 hours.";
    }
    if (["about"].includes(category)) {
      return "HarshaVardhan is a passionate developer. Check the About section on the homepage for a detailed introduction.";
    }
    ...
    if (category === "skills") {
      ...
      if (mentionedCat) {
        const filtered = skills.filter((s) =>
          s.category?.toLowerCase().includes(mentionedCat.replace(/\s+/g, "")),
        );
        if (filtered.length) formatted = formatSkillList(filtered);
      } else {
        const mentionedSkill = skills.find((s) =>
          lower.includes(s.skillName?.toLowerCase()),
        );
      ...
      const experiences = await Experience.find()
        ...
        .lean();
      const lower = message.toLowerCase();
      const mentioned = experiences.find(
        (e) =>
          lower.includes(e.companyName?.toLowerCase()) ||
          lower.includes(e.designation?.toLowerCase()),
      );
      ...
      const lower = message.toLowerCase();
      const mentioned = education.find(
        (e) =>
          lower.includes(e.instituteName?.toLowerCase()) ||
          lower.includes(e.degree?.toLowerCase()),
      );
      ...
      const certificates = await Certificate.find()
        ...
        .lean();
      const lower = message.toLowerCase();
      const mentioned = certificates.find(
        (c) =>
      ...
      const recognitions = await Recognition.find()
        ...
        .lean();
      const lower = message.toLowerCase();
      const mentioned = recognitions.find((r) =>
    ...
    const items = [];
    const [allSkills, allProjects, allExp, allEdu, allCert, allRec] =
      await Promise.all([
        Skill.find().lean(),
        Project.find().lean(),
        ...
        Recognition.find().lean(),
      ]);
    for (const s of allSkills)
      items.push({
        _type: "skill",
        _title: s.skillName,
        _detail: `${s.category || ""} | ${s.percentage}%`,
        _searchText: `${s.skillName} ${s.category || ""}`,
      });
    for (const p of allProjects)
      items.push({
        _type: "project",
        _title: p.title,
        _detail: (p.technologies || []).join(", "),
        _searchText: `${p.title} ${p.description || ""} ${(p.technologies || []).join(" ")}`,
      });
    for (const e of allExp)
      items.push({
        _type: "experience",
        _title: e.designation,
        _detail: `${e.companyName}${e.location ? ` - ${e.location}` : ""}`,
        _searchText: `${e.designation} ${e.companyName} ${e.description || ""}`,
      });
    for (const e of allEdu)
      items.push({
        _type: "education",
        _title: e.degree,
        _detail: `${e.instituteName}`,
        _searchText: `${e.degree} ${e.instituteName}`,
      });
    for (const c of allCert)
      items.push({
        _type: "certificate",
        _title: c.title,
        _detail: c.issuer || "",
        _searchText: `${c.title} ${c.issuer || ""}`,
      });
    for (const r of allRec)
      items.push({
        _type: "recognition",
        _title: r.title,
        _detail: (r.description || "").substring(0, 100),
        _searchText: `${r.title} ${r.description || ""}`,
      });

    const fuse = new Fuse(items, {
      keys: ["_searchText", "_title", "_detail"],
      threshold: 0.45,
      includeScore: true,
      minMatchCharLength: 2,
    });
    const results = fuse
      .search(message, { limit: 5 })
    ...
    if (results.length) {
      ...
      for (const [type, items] of Object.entries(grouped)) {
        ...
        for (const item of items.slice(0, 3))
          text += `   • ${item._title}${item._detail ? ` — ${item._detail}` : ""}\n`;
        text += "\n";
      }
      return text.trim();
    }
